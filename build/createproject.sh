...
src=$(git rev-parse --show-toplevel) || {
  echo "try running the script from within GoldenPhi-Boilerplate directories." >&2
  exit 1
}
[[ -d $src ]] || {
  echo "fatal: could not determine GoldenPhi-Boilerplate's root directory." >&2
...
while [[ -z $name ]]
do
    echo "To create a new GoldenPhi-Boilerplate project, enter a new directory name:"
    read name || exit
done
